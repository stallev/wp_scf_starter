...
| **M6** | Инструменты и тесты | `init`, `check-naming`, `check-hardcode`, `check-links`, `lint:prototype`, `validate-seeds` + JSON Schema, `font-fallback-metrics`, `convert-to-webp`, `psi.mjs`, `gate:*`; e2e: static, navigation, forms, dynamic, seo, a11y, perf-markup, console, visual | ✅ Выполнен (2026-09-25): M6a — инструменты (`init`, `check:hardcode`, `lint:prototype`, `check:seeds`, `fonts:fallback`, `images:webp`, `psi`, `gate:*`, 51 unit-тест); M6b — 10 e2e-наборов Playwright (127 passed, 9 skipped: visual без прототипа). e2e — низкий приоритет (D26) |
| **M7** | Документация стартера | `contracts/*`, `playbooks/*` (performance, psi, prototype-rules, architecture, analytics, launch), `phases/ROADMAP` + phase-0…7, шаблоны `docs/project/*` | ✅ Выполнен (2026-09-25): 9 контрактов, 6 playbooks, ROADMAP + 8 фаз, шаблоны `docs/project/`; K1–K18 закрыты; 299 ссылок проверены `check:links`; review пройден |
...
| D26 | e2e-тесты — низкоприоритетная часть стартера: наборы закоммичены в рабочем состоянии без отдельного review; основной контроль — статические проверки (`gate:0`, `gate:page`) | Решение владельца проекта; e2e дорого поддерживать, ценность ниже, чем у статических gate |
